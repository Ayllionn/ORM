...
print("OBJ 2 :::\n")
for k,v in data2.attr().items():
    print(f"{k} : {v}")

print("\n\n")

print("Avant")
# affiche toute un table
[print(i) for i in orm.get_all_by_table(test_instance.table)]

data2.name = "Je suis un nouveau nom"
data2.update() #pour mettre a jour les valeurs que vous changer

print("après")
[print(i) for i in orm.get_all_by_table(test_instance.table)]

data1.delete() #supprime la donné
```
